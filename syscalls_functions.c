...
#define IP_ADDRESS INADDR_LOOPBACK
#define PORT 25588
...
void * start_server_thread(void *arg) {
  ...
  setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (void*) &optVal,
             optLen);
  */
 ...
 */
int execute_dup3() {
  // run dup3 with O_CLOEXEC
  int oldfd = open_rdonly(FILENAME1);
  int newfd = open_rdwr(FILENAME2);
  dup3(oldfd, newfd, O_CLOEXEC);
 
  // run dup3 with no flag.
  dup3(oldfd, newfd, -1);
  
  // run dup3 with oldfd=newfd and O_CLOEXEC
  dup3(oldfd, oldfd, O_CLOEXEC);
...
int execute_fstatfs() {
  ...
  // open a file and run fstatfs.
  int fd = open_rdonly(FILENAME1);
  fstatfs(fd, &data);
  
  // close fd and run fstatfs again.
  close(fd);
  fstatfs(fd, &data);
...
int execute_statfs() {
  ...
  // make sure the file exists and run statfs.
  close(open_rdonly(FILENAME1));
  statfs(FILENAME1, &data);
  
  // run statfs on a non existing file.
  statfs(FILENAMEX, &data);
